...
func RegisterCommands(app *kingpin.Application) *Application {
	...
	g.EtcdRetryTimeout = g.Flag("etcd-retry-timeout", "Retry timeout for transient etcd errors.").Duration()
	g.UID = app.Flag("uid", "Effective user ID for this operation. Must be >= 0.").Default(strconv.Itoa(defaults.PlaceholderUserID)).Int()
	g.GID = g.Flag("gid", "Effective group ID for this operation. Must be >= 0.").Default(strconv.Itoa(defaults.PlaceholderGroupID)).Int()
	g.ProfileEndpoint = g.Flag("httpprofile", "Enable profiling endpoint on specified host/port i.e. localhost:6060.").String()
	g.ProfileTo = g.Flag("profile-dir", "Store periodic state snapshots in the specified directory.").String()
	...
	g.SystemLogFile = g.Flag("system-log-file", "Path to the log file with system level logs.").String()
	...
	g.InstallCmd.App = g.InstallCmd.Flag("app", "Cluster image name to install.").String()
	...
	g.InstallCmd.Mode = g.InstallCmd.Flag("mode", fmt.Sprintf("Install mode. One of: %v.",
		modules.Get().InstallModes())).Default(constants.InstallModeCLI).String()
	// Deprecated
	_ = g.InstallCmd.Flag("docker-device", "[DEPRECATED] This flag will be removed in future version.").String()
	g.InstallCmd.SystemDevice = g.InstallCmd.Flag("system-device", "Device to use for system data directory.").String()
	...
	g.InstallCmd.DNSHosts = g.InstallCmd.Flag("dns-host", "Specify an IP address that will be returned for the given domain within the cluster. Accepts <domain>/<ip> format. Can be specified multiple times.").Strings()
	...
	g.InstallCmd.FromService = g.InstallCmd.Flag("from-service", "Run in service mode.").Bool()
	...
	// Deprecated
	_ = g.JoinCmd.Flag("docker-device", "[DEPRECATED] This flag will be removed in future version.").String()
	g.JoinCmd.SystemDevice = g.JoinCmd.Flag("system-device", "Device to use for system data directory.").String()
	g.JoinCmd.ServerAddr = g.JoinCmd.Flag("server-addr", "Address of the agent server.").String()
	...
	g.JoinCmd.OperationID = g.JoinCmd.Flag("operation-id", "ID of the operation that was created via UI.").String()
	...
	g.JoinCmd.FromService = g.JoinCmd.Flag("from-service", "Run in service mode.").Bool()
	...
	// Deprecated
	_ = g.AutoJoinCmd.Flag("docker-device", "Docker device to use.").String()
	g.AutoJoinCmd.SystemDevice = g.AutoJoinCmd.Flag("system-device", "Device to use for system data directory.").String()
	...
	g.AutoJoinCmd.AdvertiseAddr = g.AutoJoinCmd.Flag("advertise-addr", "IP address this node will advertise to other cluster nodes.").String()
	g.AutoJoinCmd.Token = g.AutoJoinCmd.Flag("token", "Unique token to authorize this node to join the cluster.").String()
	...
	g.AutoJoinCmd.FromService = g.AutoJoinCmd.Flag("from-service", "Run in service mode.").Bool()
	...
	g.ResumeCmd.OperationID = g.ResumeCmd.Flag("operation-id", "ID of the active operation. It not specified, the last operation will be used.").String()
	g.ResumeCmd.SkipVersionCheck = g.ResumeCmd.Flag("skip-version-check", "Bypass version compatibility check.").Bool()
	g.ResumeCmd.Force = g.ResumeCmd.Flag("force", "Force execution of specified phase.").Bool()
	g.ResumeCmd.PhaseTimeout = g.ResumeCmd.Flag("timeout", "Phase execution timeout.").Default(defaults.PhaseTimeout).Duration()
	...
	g.StartCmd.FromService = g.StartCmd.Flag("from-service", "Run in service mode.").Bool()
	...
	g.PlanCmd.SkipVersionCheck = g.PlanCmd.Flag("skip-version-check", "Bypass version compatibility check.").Bool()
	...
	g.PlanExecuteCmd.PhaseTimeout = g.PlanExecuteCmd.Flag("timeout", "Phase execution timeout.").Default(defaults.PhaseTimeout).Duration()
	...
	g.PlanRollbackCmd.PhaseTimeout = g.PlanRollbackCmd.Flag("timeout", "Phase rollback timeout.").Default(defaults.PhaseTimeout).Duration()

	g.PlanSetCmd.CmdClause = g.PlanCmd.Command("set", "Set the specified phase state without executing it.")
	...
	g.PlanResumeCmd.PhaseTimeout = g.PlanResumeCmd.Flag("timeout", "Phase execution timeout.").Default(defaults.PhaseTimeout).Duration()
	...
	g.RollbackCmd.PhaseTimeout = g.RollbackCmd.Flag("timeout", "Individual phase rollback timeout").Default(defaults.PhaseTimeout).Duration()
	...
	g.RollbackCmd.SkipVersionCheck = g.RollbackCmd.Flag("skip-version-check", "Bypass version compatibility check").Bool()
	...
	g.UpdateCheckCmd.CmdClause = g.UpdateCmd.Command("check", "Check if an update is available for the specified cluster image.")
	...
	g.UpdateTriggerCmd.CmdClause = g.UpdateCmd.Command("trigger", "Trigger an upgrade operation for the given cluster image.")
	...
	g.UpdateTriggerCmd.SkipVersionCheck = g.UpdateTriggerCmd.Flag("skip-version-check", "Bypass version compatibility check.").Bool()
	...
	g.UpdateTriggerCmd.SkipWorkers = g.UpdateTriggerCmd.Flag("skip-workers", "Skip upgrade of worker nodes").Bool()
	...
	g.UpdatePlanInitCmd.SkipWorkers = g.UpdatePlanInitCmd.Flag("skip-workers", "Skip upgrade of worker nodes").Bool()
	...
	g.UpgradeCmd.CmdClause = g.Command("upgrade", "Trigger an upgrade operation.")
	...
	g.UpgradeCmd.Timeout = g.UpgradeCmd.Flag("timeout", "Phase execution timeout.").Default(defaults.PhaseTimeout).Duration()
	...
	g.UpgradeCmd.SkipVersionCheck = g.UpgradeCmd.Flag("skip-version-check", "Bypass version compatibility check.").Bool()
	...
	g.UpgradeCmd.SkipWorkers = g.UpgradeCmd.Flag("skip-workers", "Skip upgrade of worker nodes").Bool()
	g.UpgradeCmd.ParallelWorkers = g.UpgradeCmd.Flag("parallel-workers", "Number of worker nodes to upgrade in parallel (defaults to serial upgrade of workers)").Int()

	g.UpdateUploadCmd.CmdClause = g.UpdateCmd.Command("upload", "Upload update package to locally running site")
	...
	g.UpdateCompleteCmd.CmdClause = g.UpdateCmd.Command("complete", "Mark update operation as completed")
	...
	g.UpdateSystemCmd.CmdClause = g.UpdateCmd.Command("system", "Update this system by installing newer versions of system packages")
	g.UpdateSystemCmd.ChangesetID = g.UpdateSystemCmd.Flag("changeset-id", "Assign ID to this update operation (will be autogenerated if missing)").String()
	g.UpdateSystemCmd.ServiceName = g.UpdateSystemCmd.Flag("service-name", "The name of the service to run update as a systemd unit").String()
	...
	g.StatusResetCmd.CmdClause = g.Command("status-reset", "Force-reset the cluster state to active. USE WITH CAUTION, the cluster may end up in an inconsistent state.")
	...
	g.AppSyncCmd.ScanningRepository = g.AppSyncCmd.Flag("scan-repository", "Specify repository to copy vendored images to").Envar("GRAVITY_SCAN_REPOSITORY").String()
	g.AppSyncCmd.ScanningTagPrefix = g.AppSyncCmd.Flag("scan-prefix", "Specify a prefix for tags when copying to repository").Envar("GRAVITY_SCAN_PREFIX").String()
	...
	g.AppRebuildIndexCmd.CmdClause = g.AppCmd.Command("rebuild-index", "Rebuild Helm chart repository index.")

	g.AppIndexCmd.CmdClause = g.AppCmd.Command("index", "Generate an index file for application/cluster images.")
	...
	g.AppImportCmd.CmdClause = g.AppCmd.Command("import", "Import application into gravity")
	...
	g.AppImportCmd.Parallel = g.AppImportCmd.Flag("parallel", "specifies number of concurrent tasks. If < 0, the number of tasks is not restricted, if unspecified, then tasks are capped at the number of logical CPU cores.").Int()

	// export gravity application
	g.AppExportCmd.CmdClause = g.AppCmd.Command("export", "export gravity application")
	...
	g.AppDeleteCmd.CmdClause = g.AppCmd.Command("delete", "delete gravity application")
	...
	// list installed apps
	g.AppPackageListCmd.CmdClause = g.AppCmd.Command("package-list", "list installed applications")
	...
	g.AppPackageListCmd.ShowHidden = g.AppPackageListCmd.Flag("hidden", "show hidden apps too").Bool()
	g.AppPackageListCmd.OpsCenterURL = g.AppPackageListCmd.Flag("ops-url", "optional remote Gravity Hub URL").String()

	// uninstall app
	g.AppPackageUninstallCmd.CmdClause = g.AppCmd.Command("package-uninstall", "uninstall application")
	g.AppPackageUninstallCmd.Locator = Locator(g.AppPackageUninstallCmd.Arg("pkg", "package name with application").Required())

	// get status of an application
	g.AppStatusCmd.CmdClause = g.AppCmd.Command("status", "get app status")
	...
	g.AppPullCmd.CmdClause = g.AppCmd.Command("pull", "pull an application package from remote Gravity Hub")
	...
	g.AppPushCmd.CmdClause = g.AppCmd.Command("push", "push an application package to remote Gravity Hub")
	...
	g.AppHookCmd.CmdClause = g.AppCmd.Command("hook", "run the specified application hook")
	...
	g.AppUnpackCmd.CmdClause = g.AppCmd.Command("unpack", "unpack application resources")
	...
	g.WizardCmd.CmdClause = g.Command("wizard", "start wizard that will guide you through install process")
	...
	g.WizardCmd.FromService = g.WizardCmd.Flag("from-service", "Run in service mode").Bool()
	...
	g.AppPackageCmd.CmdClause = g.Command("app-package", "Display the name of application package from installer tarball")

	// install and access ops commands
	g.OpsCmd.CmdClause = g.Command("ops", "access Gravity Hub related commands")

	g.OpsConnectCmd.CmdClause = g.OpsCmd.Command("connect", "save credentials for remote Gravity Hub on local disk")
	...
	g.OpsDisconnectCmd.CmdClause = g.OpsCmd.Command("disconnect", "disconnect and log out from Gravity Hub")
	g.OpsDisconnectCmd.OpsCenterURL = g.OpsDisconnectCmd.Arg("ops-url", "remote Gravity Hub URL").Required().String()

	g.OpsListCmd.CmdClause = g.OpsCmd.Command("ls", "list connected Gravity Hubs")

	// TODO: move this functionality to crpcAgent
	g.OpsAgentCmd.CmdClause = g.OpsCmd.Command("agent", "Start an agent to perform a set of tasks")
	...
	// import package
	g.PackImportCmd.CmdClause = g.PackCmd.Command("import", "import file or directory into package")
	...
	// unpack package
	g.PackUnpackCmd.CmdClause = g.PackCmd.Command("unpack", "unpack package into internal 'unpacked' directory")
	...
	// export package
	g.PackExportCmd.CmdClause = g.PackCmd.Command("export", "export package to specified file")
	...
	// list packages
	g.PackListCmd.CmdClause = g.PackCmd.Command("list", "list local packages")
	...
	// delete package
	g.PackDeleteCmd.CmdClause = g.PackCmd.Command("delete", "delete a package from repository")
	...
	// configure package
	g.PackConfigureCmd.CmdClause = g.PackCmd.Command("configure", "configure a package").Interspersed(false)
	...
	g.PackCommandCmd.CmdClause = g.PackCmd.Command("command", "execute command provided by the package").Interspersed(false)
	...
	g.PackPushCmd.CmdClause = g.PackCmd.Command("push", "push package to remote Gravity Hub")
	...
	g.PackPullCmd.CmdClause = g.PackCmd.Command("pull", "pull package from remote Gravity Hub")
	...
	g.PackLabelsCmd.CmdClause = g.PackCmd.Command("labels", "change package labels")
	...
	// create a new user
	g.UserCreateCmd.CmdClause = g.UserCmd.Command("create", "create a new user")
	...
	// delete a user
	g.UserDeleteCmd.CmdClause = g.UserCmd.Command("delete", "delete a user")
	...
	g.APIKeyCreateCmd.CmdClause = g.APIKeyCmd.Command("create", "create a new api key")
	...
	g.APIKeyListCmd.CmdClause = g.APIKeyCmd.Command("list", "view user api keys")
	...
	// delete an api key
	g.APIKeyDeleteCmd.CmdClause = g.APIKeyCmd.Command("delete", "delete an api key")
	...
	// list sites
	g.SiteListCmd.CmdClause = g.SiteCmd.Command("list", "list sites")
	g.SiteListCmd.OpsCenterURL = g.SiteListCmd.Flag("ops-url", "remote Gravity Hub URL").String()

	// start
	g.SiteStartCmd.CmdClause = g.SiteCmd.Command("start", "start site controller (runs inside cluster)")
	...
	// init
	g.SiteInitCmd.CmdClause = g.SiteCmd.Command("init", "import site state from external database")
	...
	// status
	g.SiteStatusCmd.CmdClause = g.SiteCmd.Command("status", "check system status")

	// info
	g.SiteInfoCmd.CmdClause = g.SiteCmd.Command("info", "Prints local cluster information to the console")
	...
	g.SiteCompleteCmd.CmdClause = g.SiteCmd.Command("complete", "Marks the final install step as completed")
	...
	g.SiteResetPasswordCmd.CmdClause = g.SiteCmd.Command("reset-password", "reset password for local user")

	// local site
	g.LocalSiteCmd.CmdClause = g.Command("local-site", "Prints the local cluster domain name to the console")
	...
	g.SystemTeleportCmd.CmdClause = g.SystemCmd.Command("teleport", "System level operations on Teleport service")
	...
	g.SystemRotateCertsCmd.CmdClause = g.SystemCmd.Command("rotate-certs", "Renew cluster certificates on a node")
	...
	g.SystemExportCACmd.CmdClause = g.SystemCmd.Command("export-ca", "Export cluster CA, must be run on a master node")
	...
	g.SystemUninstallCmd.CmdClause = g.SystemCmd.Command("uninstall", "uninstall gravity from the host")
	g.SystemUninstallCmd.Confirmed = g.SystemUninstallCmd.Flag("confirm", "confirm uninstall").Bool()

	g.SystemPullUpdatesCmd.CmdClause = g.SystemCmd.Command("pull-updates", "Pull new package updates from the system")
	...
	g.SystemUpdateCmd.CmdClause = g.SystemCmd.Command("update", "Update this system by installing newer version of system packages")
	...
	g.SystemReinstallCmd.CmdClause = g.SystemCmd.Command("reinstall", "reinstall package on the system")
	...
	g.SystemHistoryCmd.CmdClause = g.SystemCmd.Command("history", "list system update history")

	g.SystemClusterInfoCmd.CmdClause = g.SystemCmd.Command("cluster-info", "dump kubernetes cluster info suitable for debugging")

	// ask the current active master to step down
	g.SystemStepDownCmd.CmdClause = g.SystemCmd.Command("step-down", "Ask the active master to step down")

	g.SystemRollbackCmd.CmdClause = g.SystemCmd.Command("rollback", "starts rollback")
	...
	g.SystemServiceUninstallCmd.CmdClause = g.SystemServiceCmd.Command("uninstall", "uninstall service, supply either package or service name")
	...
	g.SystemReportCmd.CmdClause = g.SystemCmd.Command("report", "collect system diagnostics and output as gzipped tarball to terminal")
	...
	g.SystemStateDirCmd.CmdClause = g.SystemCmd.Command("state-dir", "show where all gravity data is stored on the node")

	// journal helpers
	g.SystemExportRuntimeJournalCmd.CmdClause = g.SystemCmd.Command("export-runtime-journal", "Export runtime journal logs to a file")
	...
	g.SystemStreamRuntimeJournalCmd.CmdClause = g.SystemCmd.Command("stream-runtime-journal", "Stream runtime journal to stdout")
	...
	g.SystemGCJournalCmd.CmdClause = systemGCCmd.Command("journal",
		"Clean up stale journal directories. "+
			"Directories that do not match the effective systemd machine-id will be removed.")
	...
	g.PlanetCmd.CmdClause = g.Command("planet", "operations with planet")

	g.PlanetEnterCmd.CmdClause = g.PlanetCmd.Command("enter", "[DEPRECATED] enters currently installed planet")

	g.PlanetStatusCmd.CmdClause = g.PlanetCmd.Command("status", "calls status for currently installed planet")

	g.EnterCmd.CmdClause = g.Command("enter", "enter planet")
