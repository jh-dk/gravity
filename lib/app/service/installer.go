...
func pullApplications(apps []appservice.Application, localApps, remoteApps *Applications, log log.FieldLogger) error {
	...
	for _, app := range apps {
		...
		labels := utils.CombineLabels(app.PackageEnvelope.RuntimeLabels, envelope.RuntimeLabels)
		_, err = localApps.CreateAppWithManifest(app.Package, manifestBytes, reader, labels)
