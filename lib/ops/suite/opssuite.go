...
func SetUpTestPackage(c *C, apps app.Applications, packages pack.PackageService) loc.Locator {
	runtimeApp := apptest.DefaultRuntimeApplication()
	app := apptest.ClusterApplication(loc.MustParseLocator("gravitational.io/app:0.0.1"), runtimeApp)
	app.Manifest.Dependencies = schema.Dependencies{
	...
	req := apptest.AppRequest{
		App:      app,
		Packages: packages,
		Apps:     apps,
	}
	return apptest.CreateApplication(req, c).Package
}
