...
Every cluster lifecycle hook container gets injected with the following environment variables:

Name                       | Description
---------------------------|-----------------------------------------------------
`GRAVITY_CLUSTER_NAME`     | Installed cluster name, set via `--cluster` flag or autogenerated.
`GRAVITY_CLUSTER_PROVIDER` | Cloud provider name, set via `--cloud-provider` flag or autodetected. Can be `generic`, `aws` or `gce`.
`GRAVITY_CLUSTER_FLAVOR`   | Name of the flavor used for initial installation, set via `--flavor` flag.
...
Step 1. Generate Certifcate Authority (CA) and private key for 365 days:
  
```bash
openssl req -newkey rsa:2048 -nodes -keyout domain.key -x509 -days 365 -out domain.crt
```
  
Step 2. Generate a license with an encryption key using the CA
```bash
...
Step 3. Enable licensing feature in the cluster image manifest
